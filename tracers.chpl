...
proc initialize(ref Tr: Tracers, P: Params, D) {

    Tr.h[D.grid] = get_var(P.grdfile, "h", D.grid);

    Tr.H0[D.rho_3D] = get_H0(Tr.h[D.grid], P);

    // Initialize tracer fields
      Tr.tracer[D.rho_3D] = get_var(P.velfiles[P.Nt_start+1], "temp", D.rho_3D);

    // Initialize zeta and thicknesses
      update_thickness(Tr.zeta_nm1, Tr.H_nm1, Tr.H0, Tr.h, D, P, P.Nt_start);
      update_thickness(Tr.zeta_n, Tr.H_n, Tr.H0, Tr.h, D, P, P.Nt_start+1);
...
proc calc_half_step_tr(ref Tr: Tracers, D: Domains, P: Params) {
    ...
    forall (t,j,i) in D.rho_2D {
      ...
      Tr.zeta_np3h[t,j,i] = (1.5 + P.beta) * Tr.zeta_np1[t,j,i] - (0.5 + 2*P.beta) * Tr.zeta_n[t,j,i] + P.beta*Tr.zeta_nm1[t,j,i];
    }
