...
proc TimeStep(ref Dyn: Dynamics, D: Domains, ref Trc: Tracers, F: Files, step : int) {


  // Load velocity fields for the next timestep
    update_thickness(Trc.zeta_np1, Trc.H_np1, Trc.H0, Trc.h, D, F, step+1);
    update_dynamics(Dyn.U_np1, Dyn.V_np1, Trc.H_np1, D, F, step+1);
...
WriteOutput(Trc.tracer, "init", "stuff", step);
  ...
  // Calculate W at n using the continuity equation
    calc_W(Dyn.U_n, Dyn.V_n, Dyn.W_n, D, Trc.H_nm1h, Trc.H_np1h);

  // Calculate W at (n+1/2) using the continuity equation
    calc_W(Dyn.U_np1h, Dyn.V_np1h, Dyn.W_np1h, D, Trc.H_n, Trc.H_np1);

  // Calculate W at (n+1) using the continuity equation
    calc_W(Dyn.U_np1, Dyn.V_np1, Dyn.W_np1, D, Trc.H_np1h, Trc.H_np3h);

  // Calculate tracer fluxes at the n time step.
    calc_fluxes(Dyn.U_n, Dyn.V_n, Dyn.W_n, Dyn, D, Trc.tracer, Trc.H_n);

    RHS(Trc.k1, Dyn, D, Trc.H_n);

    Trc.tmp[D.rho_3D] =  Trc.tracer[D.rho_3D] + 0.5*dt*Trc.k1[D.rho_3D];
    update_halos(Trc.tmp);
    calc_fluxes(Dyn.U_np1h, Dyn.V_np1h, Dyn.W_np1h, Dyn, D, Trc.tmp, Trc.H_np1h);

    RHS(Trc.k2, Dyn, D, Trc.H_np1h);

    Trc.tmp[D.rho_3D] =  Trc.tracer[D.rho_3D] + 0.5*dt*Trc.k2[D.rho_3D];
    update_halos(Trc.tmp);
    calc_fluxes(Dyn.U_np1h, Dyn.V_np1h, Dyn.W_np1h, Dyn, D, Trc.tmp, Trc.H_np1h);

    RHS(Trc.k3, Dyn, D, Trc.H_np1h);

    Trc.tmp[D.rho_3D] =  Trc.tracer[D.rho_3D] + dt*Trc.k3[D.rho_3D];
    update_halos(Trc.tmp);
    calc_fluxes(Dyn.U_np1, Dyn.V_np1, Dyn.W_np1, Dyn, D, Trc.tmp, Trc.H_np1);

    Trc.tracer[D.rho_3D] = (Trc.tracer[D.rho_3D]*Trc.H_n[D.rho_3D] + one_sixth*dt*
...
proc RHS(ref tmp, ref Dyn: Dynamics, D: Domains, ref H) {
  ...
  if (here.id == 0) {
    forall (t,k,j,i) in {0..0, D.rho_3D.dim[1], (D.rho_3D.first[2]+1)..(D.rho_3D.last[2]-1), (D.rho_3D.first[3]+1)..D.rho_3D.last[3]} with (ref Dyn) {
      tmp[t,k,j,i] = - iarea*((Dyn.tmp_U[t,k,j,i] - Dyn.tmp_U[t,k,j,i-1])
